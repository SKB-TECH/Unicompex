...
class myPDF extends FPDF {
    ...
    function vieTable($pdo){
        ...
        while($data = $res->fetch()){
            ...
            $res4 = $pdo->query("SELECT * from recouvrements ");
            $data5 = $res4->fetch();
           	
            
            // while($data4 = $res4->fetch()){
            //     if($idDette === $data4['codeEmprunt']){
            //         $id2 = $data4['codeEmprunt'];
            //         $res6 =  $pdo->query("SELECT *  from recouvrements where codeEmprunt='$id2');
            //         $data6 = $res6->fetch();
            //         $reste = 0;
            //         break;
            //     }else{
            ...
            $this->Cell(30,7,$data['noms'],1,0,'C', true);
            $this->Ln();
            $somme = $data['dette'];
        }
        $this->Cell(165,7,"TOTAL DETTES GENERALES ",1,0,'C', true);
        $this->SetFont('times','B',12);
          $res3 = $pdo->query("SELECT sum(montant) as dette from dettes where obs=0");
          $data3 = $res3->fetch();
