...
class myPDF extends FPDF {
    ...
    function Table($pdo){
        ...
        $this->Ln(3);
        $this->cell(195,6,utf8_decode('LISTE GENERALE DES REMBOURSEMENTS  '),0,0,'C');
        $this->Ln();
    ...
    function headerTable(){
        ...
        $this->Cell(25,7,utf8_decode('MONTANT'),1,0,'C', true);
         $this->Cell(25,7,utf8_decode('CREDIT'),1,0,'C', true);
         $this->Cell(25,7,utf8_decode('AGENT'),1,0,'C', true); 
    }
    function vieTable($pdo){
        $i = 0;
        
            $res=$pdo->query("SELECT  *, sum(remboursements.montant) as somme),credits.montant as credit, reboursement.montant as remb FROM remboursement 
            inner join credit on remboursement.idCredit = credits.code_credit
            inner join membres on membres.code_membre = credit.idMembre
            inner join user on user.code_user = remboursement.idAgent
        ");
        while($data = $res->fetch()){
            ...
            $this->Cell(30,7,utf8_decode($data['code_remb']),1,0,'C', true);
            $this->Cell(80,7,$data['noms_membre'],1,0,'C', true);
            $this->Cell(20,7,$data['telephone_membre'],1,0,'C', true);
            $this->Cell(30,7,$data['date_remb'],1,0,'C', true);
            $this->Cell(25,7,$data['remb'],1,0,'C', true);
            $this->Cell(25,7,$data['credit'],1,0,'C', true);
            $this->Cell(25,7,$data['noms'],1,0,'C', true);
