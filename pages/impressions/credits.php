...
class myPDF extends FPDF {
    ...
    function Table($pdo){
        ...
        $this->Ln(3);
        $this->cell(195,6,utf8_decode('LISTE DES CREDITS AVEC  '),0,0,'C');
        $this->Ln();
    ...
    function headerTable(){
        ...
        $this->Cell(25,7,utf8_decode('MOTIF'),1,0,'C', true);
    ...
    function vieTable($pdo){
        ...
        $res=$pdo->query("SELECT  * FROM credit 
        inner join membres on memebres.code_membre = credits.idMembre and obs_credt = 0
        inner join user on user.code_user = credits.idAgent
        ");
        while($data = $res->fetch()){
            ...
            $this->Cell(30,7,utf8_decode($data['code_credit']),1,0,'C', true);
            $this->Cell(80,7,$data['noms_membre'],1,0,'C', true);
            $this->Cell(20,7,$data['telephone_membre'],1,0,'C', true);
            $this->Cell(30,7,$data['date_credit'],1,0,'C', true);
            $this->Cell(25,7,$data['montant_credit'],1,0,'C', true);
            $this->Cell(25,7,$data['motif'],1,0,'C', true);
